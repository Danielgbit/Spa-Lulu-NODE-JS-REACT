...
const appointmentsController = {
    ...
    updateAppointment: async (req, res) => {
        ...
        try {
            ...
            if (!appointment) {
            return res.status(404).json({ success: false, error: 'La cita no se ah encontrado' });
            }
            ...
            const availabilityAndAppointments = await db.AppointmentAvailability.findOne({
                where: {
                    appointment_id: id
                }
            })
            ...
            const [rowsUpdateAvailability, updateAvailability] = await db.Availability.update(availabilityUpdate, {
                where: {
                    availability_id: availabilityAndAppointments.availability_id
                }
            });
    ...
    getAppointByDay: async (req, res) => {
        try {

            const query = req.query;

            const startDate = startOfDay(query.date);
            const endDate = endOfDay(query.date);

            const appointByDay = await db.AppointmentAvailability.findAll({
                include: [
                    {
                        model: db.Appointment,
                        as: 'appointment',
            ...
            if (!appointByDay) return res.status(404).json({ message: 'No se encontraron citas en este rango de fechas' });

            const combinedData = appointByDay?.map((data) => ({
                employeeName: data.availability.availabilityEmployee.full_name,
                appointmentId: data.appointment_id,
                clientName: `${data.appointment.appointmentClient.first_name} ${data.appointment.appointmentClient.middle_name} ${data.appointment.appointmentClient.last_name}`,
                serviceName: data.appointment.appointmentService.service_name,
            ...
            res.status(200).json({ allAppointments: combinedData });
    ...
    getAppointByDate: async (req, res) => {
        try {

            const date = req.query;

            const startDate = startOfDay(date.start);
            const endDate = endOfDay(date.end);

            const appointByDate = await db.AppointmentAvailability.findAll({
                include: [
                    {
                        model: db.Appointment,
                        as: 'appointment',
            ...
            if (!appointByDate) return res.status(404).json({ message: 'No se encontraron citas en este rango de fechas' });

            const combinedData = appointByDate?.map((data) => ({
                employeeName: data.availability.availabilityEmployee.full_name,
                appointmentId: data.appointment_id,
                clientName: `${data.appointment.appointmentClient.first_name} ${data.appointment.appointmentClient.middle_name} ${data.appointment.appointmentClient.last_name}`,
                serviceName: data.appointment.appointmentService.service_name,
    ...
    getAppointByEmployee: async (req, res) => {
        try {
            const { id } = req.params;

            const appointmentByEmployee = await db.AppointmentAvailability.findAll({
                where: {
                    employee_id: id
                },
                include: [
            ...
            if (!appointmentByEmployee) return res.status(404).json({ message: 'No se encontraron citas' });

            const combinedData = appointmentByEmployee?.map((data) => ({
                employeeName: data.availability.availabilityEmployee.full_name,
                appointmentId: data.appointment_id,
                clientName: `${data.appointment.appointmentClient.first_name} ${data.appointment.appointmentClient.middle_name} ${data.appointment.appointmentClient.last_name}`,
                serviceName: data.appointment.appointmentService.service_name,
    ...
    pruebaControl: async (req, res) => {
        try {
            ...
            res.status(200).json(availabilityAndAppointments);
