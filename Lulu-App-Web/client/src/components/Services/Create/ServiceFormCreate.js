...
const ServiceFormCreate = ({ categories, createService, errors }) => {
    ...
    const handleOnSubmit = (data) => {
        createService({ ...data, image: data.image[0], price: removeSemiColons(data?.price)});
    };

    const { register, formState: {errors: err}, handleSubmit } = useForm();

    return(
        <div className='reusable-form-body' >
            <h1 className='reusable-form-title' >Creacion de servicio</h1>
            <form onSubmit={handleSubmit(handleOnSubmit)} className="reusable-form-container" >
                ...
                <div className="reusable-crud-form-text-input-wrapper"  >
                    ...
                    { err.image?.type === 'required' && <p className='message-error'>Debes seleccionar una imagen</p>}
