...
const InventoryCreateForm = ({ createInventoryItem }) => {

  const { register, formState: {errors}, handleSubmit } = useForm();

  const onSubmit = (data) => {
    createInventoryItem({ ...data, price: data.price.toString().replace(/[$.,]/g, ''), salePrice: data.salePrice.toString().replace(/[$.,]/g, '') });
  };

  return (
      <div className='reusable-form-body' >
        <h1 className='reusable-form-title' >Creación de inventario</h1>
          ...
          <div className="reusable-crud-form-text-input-wrapper">
              <select className="textInput selectedInput" name="" id="" {...register('category', {
                required: true,
                maxLength: 100,
                minLength: 2
              })}>
          ...
          { errors.description?.type === 'maxLength' && <p style={{ color: 'white', fontSize: '10px', fontFamily: 'Poppins' }} >Maximo 200 caracteres permitidos</p> }
            <div className="reusable-crud-form-text-input-wrapper">
                <input placeholder="Ingrese el precio al que se compro" type="text" style={{ fontFamily: 'Poppins', fontSize: '11px' }}  className="textInput" {...register('price', {
                    maxLength: 10,
                    pattern: /^\$?\s?(\d{1,3}(\.\d{3})*|\d+)(,\d{1,2})?$/
                })} />
            </div>
                { errors.price?.type === 'maxLength' && <p style={{ color: 'white', fontSize: '10px', fontFamily: 'Poppins' }} >Maximo permitido 10 caracteres</p> }
                { errors.price?.type === 'pattern' && <p style={{ color: 'white', fontSize: '10px', fontFamily: 'Poppins' }} >Debe ser un valor numerico</p> }
            
              <div className="reusable-crud-form-text-input-wrapper">
                <input placeholder="Ingrese el precio de venta" type="text" style={{ fontFamily: 'Poppins', fontSize: '11px' }}  className="textInput" {...register('salePrice', {
                    maxLength: 10,
                    pattern: /^\$?\s?(\d{1,3}(\.\d{3})*|\d+)(,\d{1,2})?$/
                })} />
            </div>
                { errors.salePrice?.type === 'maxLength' && <p style={{ color: 'white', fontSize: '10px', fontFamily: 'Poppins' }} >Maximo permitido 10 caracteres</p> }
